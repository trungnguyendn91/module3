...
public class FacilityRepositoryService implements IFacilityRepository {
    ...
    private final String FIND_BY_FACILITY_NAME = "select * from dich_vu where ten_dich_vu like ?;";
    ...
    public List<Facility> findByServiceName(String name) {
        ...
        try {
            Connection connection = BaseRepository.getConnectDB();
            PreparedStatement preparedStatement = connection.prepareStatement(FIND_BY_FACILITY_NAME);
            ...
            while (resultSet.next()) {
                ...
                int numberOfFloor = resultSet.getInt("so_tang");
                String freeSerVice = resultSet.getString("dich_vu_mien_phi_di_kem");
                Facility facility = new Facility(facilityId, facilityName, facilityArea, facilityCost,
                        facilityMaxPeople, rentTypeId, facilityTypeId, standardRoom, description, poolArea, numberOfFloor, freeSerVice);
                facilityList.add(facility);
            }
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public Facility findFacilityById(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                facility = new Facility(facilityId, facilityName, facilityArea, facilityCost, facilityMaxPeople, rentTypeId, facilityTypeId, standardRoom, description, poolArea, numberOfFloor, freeSerVice);
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
